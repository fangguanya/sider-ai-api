...
CHAT_MODES={
    ...
    "Grammar Improvement":Session.improve_grammar,
}
...
class SiderGUI(tk.Tk):
    TITLE="Unofficial Sider Frontend"
    FONT=(None,11,"normal")
    ICON="sider.ico"
    def __init__(self):
        super().__init__()
        ...
        def on_select_model(event):
            model=model_var.get()
            if model==SEPARATOR:
                msgbox.showinfo("",f"Invalid option: {model}, please select again!")
            self.model=model
        ...
        self.new_chat_button = ttk.Button(bottom_frame, text="New Chat",
                                          command=self.new_chat)
        self.new_chat_button.pack(side=tk.RIGHT, padx=5)
        self.send_button = ttk.Button(bottom_frame, text="Send",
    ...
    def update_remain(self):
        self.lbl_remain["text"]=f"""Remaining: Basic {self.session.remain or 0}/{self.session.total or 0} \
Advanced {self.session.advanced_remain or 0}/{self.session.advanced_total or 0}"""
